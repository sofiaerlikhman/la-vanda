...
> ## ⚠ Branch `landing` — one-page preview, not a shop
>
> This branch contains **a single public page**: the home page and the atelier
> page merged into one landing page. It shows the shop, a selection of
> bouquets, opening hours, the team, workshops and contact details — but
> **ordering, reserving, booking and subscribing are not possible**.
>
> All shop routes (catalogue, product, cart, checkout, account, subscription,
> voucher, configurator, request forms) are deleted on this branch, not merely
> hidden — so the ordering path cannot be reached through a URL either. What
> remains is `/impressum`, `/datenschutz` and `/barrierefreiheit`; the first
> two are legally required for a publicly reachable site in Germany even
> without a shop.
>
> **The complete shop lives on `main`.** The text below describes that
> complete shop and applies to `main` — so it applies to this branch only
> where it covers shared foundations (design tokens, i18n, build, deployment).
>
...
> accident.
>
> Currently: **`LIVE_BRANCH: landing`** — the one-page preview is live.
>
> **Back to the complete shop:** set `LIVE_BRANCH` to `main` in `deploy.yml` —
> on **both** branches, so the two copies of the file agree — and push:
>
> ```bash
> git checkout main
...
> ```
>
> The first push starts the deploy; the second only keeps the file in sync.
> The other way round (back to the landing page) works the same, with
> `landing` instead of `main`.
>
> **Publish once without changing anything:** Actions tab → "Deploy to GitHub
> Pages" → "Run workflow" → type the branch name into the "branch" field.
> That applies to that run only; the next push publishes `LIVE_BRANCH` again.
>
> Nothing is lost in the process: `main` stays unchanged with all its pages
> and features, whatever is live.
>
> → **[`BACKEND.md`](BACKEND.md)** lists every element that would need a
> backend, what it does instead, and what is still missing before publication
> (photos, map, confirmed phone number and email address).
> The same places are commented with `BACKEND` in the code:
> `grep -rn "BACKEND" src/`.
...
src/
  ...
  data/
    ...
    konfigurator.ts         Flower/greenery/wrapping/card options for the configurator
  styles/
...
**Legal** (`/impressum`, `/agb`, `/datenschutz`, `/widerruf`, `/barrierefreiheit`) — shared template (`src/components/textseite/LegalPage.tsx`), structured according to common German law, reusing real facts where the project already fixes them (address, phone, payment methods, the withdrawal exception for freshly bound goods); everything company-specific (legal form, commercial register, VAT ID, responsible person) is marked as a placeholder. Every page carries a visible notice that this does not replace review by a lawyer.
...
- **Real search instead of a no-op.** The search button in the header previously had no function (see the old gap list) — now the header icon AND the tab bar open the same search input, which leads to `/sortiment?q=…`; `SortimentBrowser` filters there for real (name/description, client-side) instead of faking a result.
...
- **Account: tabs become a list.** A literal requirement from the wireframes ("on mobile the account tabs become a list; each item is its own view") — on mobile `KontoView` shows either the list (orders/subscription/addresses/details) or the selected view with a "← Übersicht" way back, never both side by side as on desktop.
...
- `src/data/products.ts`, `accessories.ts`, `occasions.ts`, `konfigurator.ts` export `async` functions throughout, even though they currently only filter fixed arrays — precisely so they can be replaced one by one with real database queries/API calls without changing the calling pages.
...
- **Subscriptions.** `src/app/abo/AboConfigurator` only creates a one-off cart line on completion — real recurring billing needs a subscription product at a payment provider (e.g. Stripe Billing) plus a `subscriptions` table.
