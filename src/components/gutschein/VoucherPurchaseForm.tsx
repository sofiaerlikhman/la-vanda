...
export default function VoucherPurchaseForm() {
  ...
  function handleAddToCart() {
    ...
    addItem({
      ...
      kind: "product",
      name: `Gutschein ${formatPriceEUR(amountCents)}`,
      priceCents: totalCents,
      image: "Gutscheinkarte auf Werkbank, 4:3",
      meta: `${deliveryOption.title}${greeting.trim() ? " · mit Grußtext" : ""}`,
    });
    setAdded(true);
  ...
  return (
    <div>
      <p className={styles.label}>Betrag</p>
      <div className={styles.chipRow} role="group" aria-label="Betrag wählen">
      ...
      <label className={styles.customField}>
        <span>
          Eigener Betrag, {MIN_CUSTOM_EUROS} bis {MAX_CUSTOM_EUROS} €
        </span>
        <input
          type="text"
          inputMode="decimal"
        ...
        {customIsSet && !customValid && (
          <span className={styles.customError}>
            Bitte einen Betrag zwischen {MIN_CUSTOM_EUROS} und {MAX_CUSTOM_EUROS} € eingeben.
          </span>
        )}
      </label>
      ...
      <div className={styles.deliveryList} role="group" aria-label="Zustellung wählen">
        {DELIVERY_OPTIONS.map((option) => {
          ...
          return (
            ...
            >
              ...
              <span className={styles.deliveryPrice}>{option.priceLabel}</span>
            </button>
          );
        })}
      </div>
      ...
      <div className={styles.actions}>
        ...
        <span className={styles.summary}>
          {canAdd ? `${formatPriceEUR(totalCents)} ${deliveryOption.summaryLabel}` : "Betrag eingeben"}
        </span>
      </div>
    </div>
  );
